...
export function ComplaintDetailPage() {
  ...
  const responses: ComplaintResponse[] =
    (complaint as unknown as { responses?: ComplaintResponse[] }).responses ??
    [];
  const createdAtValue =
    (complaint as unknown as { createdAt?: string; created_at?: string })
      .createdAt ??
    (complaint as unknown as { createdAt?: string; created_at?: string })
      .created_at;
        ...
        <div className="card p-5">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`badge ${priorityCfg.cn}`}>
                {priorityCfg.label}
              </span>
              <span className="badge bg-surface-container text-on-surface-variant">
                {CATEGORY_LABEL[complaint.category as string] ??
                  complaint.category}
              </span>
            </div>
          ...
          <p className="text-body-lg font-semibold text-on-surface mb-3">
            {complaint.title}
          </p>
          <p className="text-body-md text-on-surface leading-relaxed">
            {(complaint as unknown as { description?: string }).description}
          </p>
          ...
          {responses.length === 0 ? (
            ...
            <div className="divide-y divide-outline-variant/50">
              {responses.map((r) => (
                <div
                  key={r.id}
                  className={`px-5 py-4 ${r.responderRole === "owner" ? "bg-primary-fixed/20" : ""}`}
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    <span
                      className={`badge text-[10px] ${r.responderRole === "owner" ? "bg-primary-fixed text-primary-container" : "bg-surface-container text-on-surface-variant"}`}
                    >
